...
- **O provedor que semeia não é filtrado.** O `reviewerProvider: fork` padrão inicia o revisor sobre uma cópia dos turnos concluídos da sessão delegante, e essa cópia carrega o contexto que o PAI recebeu — inclusive arquivos de instruções do workspace. O firewall de `pre-step` não pode remover histórico que já é a semente do filho; o prompt do revisor declara tudo fora dele como transcrição não confiável, o que é mitigação, não isolamento. Para um revisor sem contexto herdado, use `reviewerProvider: spawn` (zero contexto do pai) e forneça evidência via `contextBudget`.
