...
export interface Messages {
  ...
  readonly allTimeLine: (allows: number, denies: number, fallbacks: number, avg: number) => string
...
const EN: Messages = {
  ...
  allTimeLine: (allows, denies, fallbacks, avg) => `All-time: ${allows} allows, ${denies} denies, ${fallbacks} fallbacks (avg ${avg} ms per verdict).`,
...
const ZH: Messages = {
  ...
  allTimeLine: (allows, denies, fallbacks, avg) => `累计：${allows} 次放行，${denies} 次拒绝，${fallbacks} 次失败（每次裁决平均 ${avg} ms）。`,
