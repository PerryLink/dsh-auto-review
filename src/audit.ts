 ...
 * and the `0.1.2-rc.1` / `0.1.3-alpha.2` / `0.1.5-alpha.1` hosts keep the
 * `ignorable` field on the event envelope but have no append option that
 * writes it (the third parameter is `SurfaceIntent` for surface event types
 * only — the alpha.2 and alpha.5 `append` still assemble the envelope without
 * the marker, verified against the published tarballs), so writing
 ...
 *   and the non-stamping `0.1.2+`/`0.1.3+`/`0.1.5+` prerelease lines
 *   (`0.1.2-alpha.1` through the published `0.1.2-rc.1`, `0.1.3-alpha.2`, and
 *   `0.1.5-alpha.1`).
 */
export function isUnmarkedHostVersion(version: string): boolean {
  ...
  if (rc !== null) {
    ...
    // non-stamping too (verified against each published tarball).
