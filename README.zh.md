...
| 方面 | 状态 |
|---|---|
| Harness | DeepSeek Harness `0.1.0-rc.8`（peer 依赖锁定在 `0.1.0-rc.8`） |
| Node | `^22.19.0 \|\| >=24.0.0` |
...
- **会话日志**：`autoReview/*` 事件携带审查器身份、裁决、理由、风险与耗时 —— 以信封 `ignorable: true` 标记追加，任何构建都能加载日志。`Session.append` 早于该标记的宿主（迄今发布的所有 rc 版本，至 `0.1.0-rc.8`——任何发布版都还未盖标记）会在首次追加前被探测出来（peer 版本预检 + 返回信封探测），审计降级为内存镜像 + 无标记反馈，会话日志始终保持可加载。
