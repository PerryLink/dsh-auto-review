...
- **El proveedor que siembra no se filtra.** El `reviewerProvider: fork` por defecto arranca al revisor sobre una copia de los turnos completados de la sesión delegante, y esa copia lleva el contexto que recibió el PADRE — archivos de instrucciones del espacio de trabajo incluidos. El cortafuegos de `pre-step` no puede eliminar un historial que ya es la semilla del hijo; el prompt del revisor declara todo lo ajeno a él como transcripción no confiable, lo cual es una mitigación, no aislamiento. Para un revisor sin contexto heredado, usa `reviewerProvider: spawn` (cero contexto del padre) y dale evidencia mediante `contextBudget`.
