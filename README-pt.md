...
[![dshfind](https://dshfind.com/api/badge/PerryLink/dsh-auto-review?metric=downloads&lang=pt)](https://dshfind.com/pt/plugins/PerryLink/dsh-auto-review?ref=badge)
...
| Superfície | Status |
|---|---|
| Harness | DeepSeek Harness `dsh-v0.1.7-alpha.1` (verificado em 2026-09-22). Suporte npm de linha dupla: pins de desenvolvimento e dependências de runtime `0.1.7-alpha.1`, peers `>=0.1.2-rc.1 <0.2.0 \|\| >=0.1.5-alpha.1 <0.2.0 \|\| >=0.1.6-0 <0.2.0 \|\| >=0.1.7-0 <0.2.0 \|\| >=0.1.7-0 <0.2.0` — o código do plugin detecta as linhas de host publicadas (detecção de recursos) e cada linha executa a cadeia completa de gates; as dependências de runtime seguem a linha do host para que uma instalação em um perfil nunca sombreie a árvore do host. `pnpm-workspace.yaml` fixa todo o grafo `@deepseek-ai/dsh-*` nessa linha, porque `autoInstallPeers` caso contrário preenche os peers `^0.1.1-rc.2` do subgrafo congelado `dsh-agent-spine-demo` com previews que não têm as exportações que os pacotes 0.1.7 importam. Na linha alpha.2 os fixtures de eval fixam `deepseek-flash` (o id removido `deepseek-v4-flash` não aparece mais em `eval/`). |
| Node | `^22.19.0 \|\| >=24.0.0` |
...
pnpm test                   # vitest: 25 arquivos spec
