...
- `test/` — vitest; real `Context` + real `Session`/`ApprovalService`/`InvariantRegistry` from the pinned `0.1.6-alpha.2` dev peers (the runtime `dependencies` follow the same `0.1.6-alpha.2` line — pinning an older line there hoists that tree over a newer CLI profile and crashes boot: with `0.1.5-rc.2` pins on an alpha.2 host the hoisted `dsh-home-paths` lacked `dshCachePath`, which the alpha.2 host packages import; the runtime still handles older host lines through feature detection), scripted subagent/commands/tools mocks.
