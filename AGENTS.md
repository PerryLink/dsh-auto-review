...
- `test/` — vitest; real `Context` + real `Session`/`ApprovalService`/`InvariantRegistry` from the pinned `0.1.1-rc.2` peers, scripted subagent/commands/tools mocks.
...
`scripts/check-host-versions.mjs` (run by the CI job `host-compat`) fails when the `@deepseek-ai/dsh-*` peer pins (exact `0.1.0-rc.N` or the `>=0.1.0-rc.N <0.2.0` range) no longer cover the newest rc line published by `@deepseek-ai/dsh` (on `latest` or `next`) — bump the pins (or document a deliberate stay-behind) before publishing.
